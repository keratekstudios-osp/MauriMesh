...
echo ""
echo "============================================================"
echo "MAURIMESH PROOF VAULT HEALTH + EXPORT v1"
echo "============================================================"
echo "Goal:"
echo "- Add vault health screen"
echo "- Read proof/vault AsyncStorage keys"
echo "- Create exportable integrity report"
echo "- Add dashboard button"
echo "- Do not claim native BLE/GATT PASS"
echo "============================================================"

ROOT="$(pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="$ROOT/backup-before-proof-vault-health-export-v1-$STAMP"
REPORT="$ROOT/docs/learner/proof-vault-health-export-v1-$STAMP.md"

mkdir -p "$BACKUP" "$ROOT/docs/learner" "$ROOT/app"
...
for f in app/dashboard.tsx app/proof-vault-health.tsx; do
  if [ -f "$ROOT/$f" ]; then
...
cat > "$ROOT/app/proof-vault-health.tsx" <<'TSX'
...
function parseEntry(key: string, value: string | null): VaultEntry {
  ...
  try {
    ...
    const match = safeValue.match(/packetId=([A-Z0-9-]+)/) || key.match(/(MM3-[A-Z0-9]+-[A-Z0-9]+|MMSF-[A-Z0-9]+-[A-Z0-9]+|MM-[A-Z0-9]+-[A-Z0-9]+)/);
...
if not p.exists():
    print("No dashboard found. Skipping.")
    raise SystemExit(0)

s = p.read_text()
if "/proof-vault-health" in s:
    print("Proof Vault Health button already present.")
    raise SystemExit(0)

button = '''
        <MauriButton
          title="Proof Vault Health"
          onPress={() => router.push("/proof-vault-health")}
        />
'''

idx = s.find('router.push("/learner-core")')
if idx != -1:
    end = s.find("/>", idx)
    if end != -1:
        s = s[:end+2] + "\n" + button + s[end+2:]
        p.write_text(s)
        print("Inserted after Learner Core button.")
        raise SystemExit(0)

close_idx = s.rfind("</View>")
if close_idx != -1:
    s = s[:close_idx] + button + "\n" + s[close_idx:]
    p.write_text(s)
    print("Inserted before final View close.")
    raise SystemExit(0)

print("Could not safely insert dashboard button.")
raise SystemExit(1)
PY
...
grep -n "Proof Vault Health\|proof-vault-health" app/dashboard.tsx || true
...
echo ""
echo "============================================================"
echo "PROOF VAULT HEALTH + EXPORT v1 COMPLETE"
echo "============================================================"
echo "Backup: $BACKUP"
echo "Report: $REPORT"
echo "Route: /proof-vault-health"
echo "============================================================"
