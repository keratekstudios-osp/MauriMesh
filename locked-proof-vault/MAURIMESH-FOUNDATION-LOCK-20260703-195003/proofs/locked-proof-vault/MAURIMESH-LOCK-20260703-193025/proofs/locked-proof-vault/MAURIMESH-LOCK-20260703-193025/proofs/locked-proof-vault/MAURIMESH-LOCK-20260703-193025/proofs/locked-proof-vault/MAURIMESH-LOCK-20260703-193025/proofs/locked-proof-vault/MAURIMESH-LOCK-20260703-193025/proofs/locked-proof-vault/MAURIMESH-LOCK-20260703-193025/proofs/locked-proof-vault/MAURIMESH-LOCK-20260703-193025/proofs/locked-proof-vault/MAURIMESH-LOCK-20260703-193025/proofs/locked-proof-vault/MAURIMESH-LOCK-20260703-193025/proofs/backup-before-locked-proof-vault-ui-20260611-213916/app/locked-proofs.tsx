...
import {
  LOCKED_PROOFS,
  getLockedProofReport,
} from "../src/maurimesh/proofs/lockedProofRegistry";

const BUILD_MARKER = "LOCKED_PROOFS_UI_20260612";

export default function LockedProofsScreen() {
  const passed = LOCKED_PROOFS.filter((proof) => proof.status === "PASSED").length;

  async function copyReport() {
    const report = [
      "MAURIMESH LOCKED PROOF REPORT",
      `Build marker: ${BUILD_MARKER}`,
      `Passed proofs: ${passed}/${LOCKED_PROOFS.length}`,
      "",
      getLockedProofReport(),
    ].join("\n");
    ...
    try {
      ...
      Alert.alert("Locked Proof Report", report);
    }
  }
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.summary}>
        <Text style={styles.summaryText}>PASSED PROOFS</Text>
        <Text style={styles.summaryNumber}>
          {passed}/{LOCKED_PROOFS.length}
        </Text>
        <Text style={styles.summarySub}>
          These are locked proof records. They are not live reruns.
        </Text>
      </View>

      {LOCKED_PROOFS.map((proof) => (
        <View key={proof.proofId} style={styles.card}>
          ...
          <Text style={styles.route}>{proof.route}</Text>

          <Text style={styles.label}>Proof Class</Text>
          <Text style={styles.route}>{proof.proofClass}</Text>
      ...
      <TouchableOpacity style={styles.primary} onPress={copyReport}>
        <Text style={styles.primaryText}>Copy Locked Proof Report</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  summaryText: { color: "#9EF5CF", fontWeight: "900", fontSize: 12 },
  summaryNumber: {
    color: "#FFFFFF",
    fontSize: 44,
    fontWeight: "900",
  ...
  summarySub: { color: "#AFC9BF", marginTop: 6, fontWeight: "700" },
  card: {
    marginTop: 16,
    padding: 16,
    borderRadius: 18,
  ...
  route: { color: "#D8FFF0", fontSize: 14, fontWeight: "700", marginTop: 4 },
  stage: {
    marginTop: 10,
    padding: 12,
    borderRadius: 14,
  ...
  stageSummary: { color: "#B9D2C8", fontWeight: "700", marginTop: 4 },
  primary: {
    marginTop: 18,
