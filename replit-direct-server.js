...
<body>
  <header><div style="font-size:30px;color:rgba(255,255,255,.72)">☰</div><div class="dot"></div><strong style="font-size:24px">MauriMesh</strong></header>
  <main>
    <section class="hero">
      <span class="pill">DIRECT REPLIT SERVER · NO API WAIT</span>
      <h1>MauriMesh Messenger</h1>
      <p>This direct Node server bypasses the old cached React bundle and proves the Replit static UI layer is reachable.</p>
    </section>
...
const server = http.createServer((req, res) => {
  ...
  if (req.url.startsWith("/api/health")) {
    ...
    res.end(JSON.stringify({ ok: true, service: "maurimesh-direct-server", port: PORT }));
    return;
  }
