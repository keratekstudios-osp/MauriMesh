...
echo ""
echo "============================================================"
echo "FORCE INSTALL FULL MESH TEST REPORT INTO REAL PROJECT ROOT"
echo "Target: /home/runner/workspace/app/full-mesh-test-report.tsx"
echo "============================================================"
echo ""

ROOT="/home/runner/workspace"
APP="$ROOT/app"
REPORT="$APP/full-mesh-test-report.tsx"
BACKUP="$ROOT/backup-before-force-real-full-mesh-report-$(date +%Y%m%d-%H%M%S)"

cd "$ROOT"

if [ ! -f "$ROOT/package.json" ]; then
  echo "ERROR: package.json missing in $ROOT"
  exit 1
fi

if [ ! -d "$APP" ]; then
  echo "ERROR: app folder missing in $ROOT"
  exit 1
fi
...
if [ -f "$REPORT" ]; then
  cp "$REPORT" "$BACKUP/full-mesh-test-report.tsx.bak"
fi

echo "Writing real route file..."
cat > "$REPORT" <<'TSX'
...
const checks: Check[] = [
  ...
  },
  {
    ...
    detail: "The missing required route /full-mesh-test-report is now installed in the real project root.",
    ...
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      <View style={styles.hero}>
        ...
        <Text style={styles.text}>
          /full-mesh-test-report is now present in the real project root. This fixes
          the missing required route. Real BLE ACK, relay, native telemetry, Pixel
          Calling audio, and Rust JNI still require device proof.
        </Text>
      </View>
      ...
      <View style={styles.truth}>
        ...
        <Text style={styles.text}>
          This screen proves the route is bundled. It does not by itself prove real
          BLE TX/RX, ACK, native telemetry, relay delivery, Rust JNI, or live audio calling.
        </Text>
      </View>
...
if [ -f pnpm-lock.yaml ] && command -v pnpm >/dev/null 2>&1; then
  ...
  echo "WARN: no npm or pnpm found."
fi
...
echo "============================================================"
echo "DONE"
echo "Installed into REAL project:"
pwd
echo "Route:"
echo "$REPORT"
echo "Backup:"
