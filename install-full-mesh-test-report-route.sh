...
echo ""
echo "============================================================"
echo "INSTALL FULL MESH TEST REPORT ROUTE"
echo "Target route: /full-mesh-test-report"
echo "Goal: fix missing required route and raise report readiness"
echo "============================================================"
echo ""
  ...
  return (
    <ScrollView style={styles.safe} contentContainerStyle={styles.content}>
      ...
      <View style={styles.actions}>
        ...
        <Pressable style={styles.buttonAlt} onPress={() => router.push("/test-layer")}>
...
echo "Created: app/full-mesh-test-report.tsx"

# Safely add a dashboard button when a dashboard exists and expo-router useRouter pattern is present.
if [ -f "$DASH" ]; then
  if grep -q 'full-mesh-test-report' "$DASH"; then
    echo "Dashboard already references /full-mesh-test-report"
  elif grep -q 'router.push' "$DASH"; then
    python3 - <<'PY'
...
button = '''
        <MauriButton
          title="Full Mesh Test Report"
          onPress={() => router.push("/full-mesh-test-report")}
        />
'''

# Insert before Settings button when possible, otherwise before closing common grid/view.
if 'title="Settings"' in text:
    idx = text.find('title="Settings"')
    insert_at = text.rfind("<MauriButton", 0, idx)
    if insert_at != -1:
        text = text[:insert_at] + button + text[insert_at:]
    else:
        text += "\\n// Full Mesh Test Report route: /full-mesh-test-report\\n"
else:
    marker = "</View>"
...
PY
    echo "Dashboard button insertion attempted."
  else
    echo "Dashboard found, but router.push pattern not found. Route file still installed."
  fi
else
  echo "Dashboard not found. Route file still installed."
fi

echo ""
echo "============================================================"
echo "VERIFY ROUTE FILE"
echo "============================================================"
...
if command -v pnpm >/dev/null 2>&1 && [ -f pnpm-lock.yaml ]; then
  ...
  echo "No npm/pnpm found for TypeScript check."
fi

echo ""
echo "============================================================"
echo "NEXT APK PROOF COMMANDS"
echo "============================================================"
cat <<'TXT'
After rebuild/install APK, open:
...
adb logcat -c
adb logcat | grep -Ei "AndroidRuntime|FATAL|ReactNativeJS|MauriMesh|BLE|ACK|TX_BLE|RX_BLE"

Truth:
- This fixes the missing route failure.
- It does not prove real BLE ACK yet.
- It does not prove Rust APK bridge yet.
- It does not prove Pixel Calling audio yet.
TXT

echo ""
echo "============================================================"
echo "DONE"
echo "Installed /full-mesh-test-report"
