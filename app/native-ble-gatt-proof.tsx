 ...
 * - 3-device-ready stage plan
 * - no final PASS claim from UI alone
 */
type GuidedRole = "PHONE_A" | "PHONE_B" | "PHONE_C" | "";
...
const GUIDED_4_DEVICE_STAGES: GuidedStage[] = [
  ...
  { id: 7, role: "PHONE_C", label: "Relay to PHONE_C Top Hop", marker: "NATIVE_RELAY_A16_TO_PHONE_C", color: "#B76CFF" },
  { id: 8, role: "PHONE_C", label: "PHONE_C Receive Top Hop", marker: "NATIVE_RX_PHONE_C_FROM_A16", color: "#FF7A18" },
  { id: 9, role: "PHONE_C", label: "PHONE_C ACK Back", marker: "NATIVE_ACK_PHONE_C_TO_A16", color: "#FF7A18" },
...
const AUTO_3_DEVICE_EXAM_MODE = {
  enabled: true,
  requiredPhones: ["PHONE_A", "PHONE_B", "PHONE_C"],
  removedPhones: [],
  flow: [
    "PREPARE_SHARED_PACKET_ID",
    "START_CAPTURE",
    "APPLY_PHONE_A_SENDER",
    "APPLY_PHONE_B_RELAY",
    "APPLY_PHONE_C_RECEIVER_ACK",
    "TRIGGER_NATIVE_GATT_PACKET_PAYLOAD",
    "SAVE_PROOF_TO_VAULT",
    "VERIFY_LOGCAT_MARKERS"
  ],
  requiredMarkers: [
    "BUTTON_PRESS_NATIVE_GATT_TRIGGER",
    "GATT_PACKET_PAYLOAD",
...
const MAURIMESH_AUTO_3_DEVICE_PROOF = {
  enabled: true,
  phones: ["PHONE_A", "PHONE_B", "PHONE_C"],
  removed: ["PHONE_D"],
  oneButtonLabel: "START 3-DEVICE AUTO PROOF",
  steps: [
    "GENERATE_PACKET_ID",
    "APPLY_SHARED_PACKET_ID",
    "START_CAPTURE",
    "START_RECEIVER",
    "TRIGGER_NATIVE_GATT_PACKET_PAYLOAD",
    "SAVE_ATTEMPT_TO_VAULT",
    "VERIFY_LOGCAT_MARKERS"
  ],
  requiredMarkers: [
    "BUTTON_PRESS_NATIVE_GATT_TRIGGER",
...
const MAURIMESH_FINAL_PASS_REQUIRED_MARKERS_V1 = [
  "SHARED_PACKET_V9_APPLIED",
  "BUTTON_PRESS_START_CAPTURE",
  "BUTTON_PRESS_START_REAL_GATT_RECEIVER",
  "GATT_CLIENT_WRITE_ATTEMPT",
  "GATT_PACKET_PAYLOAD",
  "GATT_SERVER_WRITE_RECEIVED",
  "NATIVE_ACK_RELAY_S10_TO_A06",
  "BUTTON_PRESS_SAVE_ATTEMPT",
];

function maurimeshHasFinalPassMarkersV1(events: any[], packetId?: string) {
  const text = events.map((e) => JSON.stringify(e)).join("\n");
  const hasMarkers = MAURIMESH_FINAL_PASS_REQUIRED_MARKERS_V1.every((m) =>
    text.includes(m)
  );
  const hasPacket = !packetId || text.includes(packetId);
  return hasMarkers && hasPacket;
}
...
export default function NativeBleGattProofScreen() {
  ...
  function guidedAdvance(expectedMarker?: string) {
    ...
    const guidedAcceptedLine =
      `MAURIMESH_GUIDED_TRUTH_GATE GUIDED_STEP_ACCEPTED step=${guidedCurrent.id} role=${guidedRole} marker=${expectedMarker || guidedCurrent.marker} packetId=${packetId} finalPassClaimed=false`;

    console.log(guidedAcceptedLine);

    const nextEventsForFinalPass = [guidedAcceptedLine, ...events];

    setEvents((prev) => [guidedAcceptedLine, ...prev].slice(0, 80));

    try {
      const finalReady = maurimeshHasFinalPassMarkersV1(
        nextEventsForFinalPass,
        packetId
      );
      ...
      console.log(
        `MAURIMESH_FINAL_PASS_PROMOTE_ERROR_V2 error=${err instanceof Error ? err.message : String(err)} finalPassClaimed=false`
      );
    }
...
const [scanActive, setScanActive] = useState(false);
  ...
  const saveAttempt = useCallback((fromAutoProof = false) => {
    ...
    const attempt: ProofAttempt = {
      ...
      nativeTriggerResult,
      nativeTriggerError,
      finalPassClaimed: finalPassClaimedForVault,
    };

    const vaultCount = saveAttemptInMemory(attempt);
    setVaultSaved(true);
    if (finalPassClaimedForVault) {
      console.log(`${LOG_TAG} MAURIMESH_VAULT_FINAL_PASS_SYNC_V5 packetId=${packetId} finalPassClaimed=true`);
    }
    if (finalPassClaimed) {
      console.log(`${LOG_TAG} MAURIMESH_VAULT_FINAL_PASS_SYNC_V4 packetId=${packetId} finalPassClaimed=true`);
    }
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={guidedStyles.card}>
        ...
        <View style={guidedStyles.roleRow}>
          {(["PHONE_A", "PHONE_B", "PHONE_C"] as GuidedRole[]).map((r) => (
            <Pressable
              key={r}
      ...
      <View style={[guidedStyles.card, guidedDimmed && guidedStyles.dimmed]}>
        ...
        {!guidedComplete && guidedCurrent ? (
          ...
          <Text style={guidedStyles.passReady}>READY FOR ONE-BUTTON PROOF</Text>
        )}
