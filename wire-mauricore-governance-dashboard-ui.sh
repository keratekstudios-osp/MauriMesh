...
echo ""
echo "============================================================"
echo "WIRE MAURICORE GOVERNANCE DASHBOARD UI"
echo "Adds route + dashboard button safely"
echo "============================================================"
echo ""

ROOT="$(pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="$ROOT/backup-before-mauricore-governance-ui-$STAMP"

mkdir -p "$BACKUP" "$ROOT/app" "$ROOT/src/mauricore/dashboard"
...
echo ""
echo "1. Create MauriCore Governance screen component"
  ...
  return (
    <ScrollView style={styles.safe} contentContainerStyle={styles.content}>
      ...
      <Card title="Build Readiness">
        ...
        {data.build.missing.slice(0, 8).map((item) => (
...
echo ""
echo "2. Create Expo Router route"
...
echo ""
echo "3. Attempt safe dashboard button injection"
...
if (!target) {
  console.log("No dashboard file found for auto injection.");
  console.log("Route still created: /mauricore-governance");
  process.exit(0);
}

let source = fs.readFileSync(target, "utf8");

if (source.includes("mauricore-governance") || source.includes("MauriCore Governance")) {
  console.log("Dashboard already contains MauriCore Governance button.");
  process.exit(0);
}

const backup = `${target}.before-mauricore-governance-${Date.now()}.bak`;
...
if (!source.includes("Pressable")) {
  source = source.replace(
    /from "react-native";/,
    (m) => m.replace("{", "{ Pressable,")
  );
}

if (source.includes("const router = useRouter()") === false) {
  source = source.replace(
...
const backHomeIndex = source.lastIndexOf("Back Home");
if (backHomeIndex !== -1) {
  const pressableBeforeBackHome = source.lastIndexOf("<", backHomeIndex);
  if (pressableBeforeBackHome !== -1) {
    source = source.slice(0, pressableBeforeBackHome) + buttonBlock + "\n" + source.slice(pressableBeforeBackHome);
    changed = true;
  }
}

if (!changed) {
  const lastViewClose = source.lastIndexOf("</");
  if (lastViewClose !== -1) {
    source = source.slice(0, lastViewClose) + buttonBlock + "\n" + source.slice(lastViewClose);
    changed = true;
  }
}
...
if (changed) {
  ...
  console.log("Could not safely inject button.");
  console.log("Route still created: /mauricore-governance");
}
NODE
...
npm run mauricore:check || npx tsc --noEmit

echo ""
echo "5. Run MauriCore smoke test"
npm run mauricore:test

echo ""
echo "============================================================"
echo "MAURICORE GOVERNANCE UI WIRING COMPLETE"
echo "Route:"
echo "  /mauricore-governance"
echo ""
echo "If button does not appear, manually open route or add button:"
echo '  router.push("/mauricore-governance")'
echo "============================================================"
